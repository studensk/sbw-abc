...
## Read in and clean observation data; EPSG 6623 is the mapping for measurements in metres in Quebec
# Allows us to calculate distances in metres
#l2 <- as.data.frame(readOGR('L2.dbf'))
l2 <- read_sf('L2.dbf') %>%
  st_transform(crs = CRS('+init=epsg:6623'))
# names(l2)[ncol(l2) - 1:0] <- c('Longitude', 'Latitude')
# cord.dec <- SpatialPoints(cbind(l2$Longitude, l2$Latitude), proj4string = CRS("+proj=longlat"))
# cord.utm <- as.data.frame(spTransform(cord.dec, CRS('+init=epsg:6623')))
# names(cord.utm) <- c('x', 'y')
# l2 <- cbind(l2, cord.utm)
...
# rast.df <- cbind(all.mat, subset(l2, select = c(x, y)))
# rast.df2 <- rast.df %>% st_as_sf(coords = c('x','y'))
...
obs <- raster::stack("sbw_defol_stack.grd")
...
rs.all <- read.csv('res_simul_all.csv', header = T)
...
write.csv(comp.df, 'Hysplit-Project/new_results_test.csv', row.names = FALSE)
#write.csv(comp.df, 'Hysplit-Project/new_results.csv', row.names = FALSE)
#write.csv(comp.df, 'results2022_full.csv', row.names = FALSE)
